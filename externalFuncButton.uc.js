...
var gExternalFuncButton = {
  autohideEmptySubDirs : true, //自动隐藏没有一个子项目的子目录菜单
	...
	init : function () {
		...
		ExternalFuncBtn = document.createElement("toolbarbutton");
		ExternalFuncBtn.setAttribute("id", "ExternalFuncButton-btn");
		...
		//navBar.insertBefore(ExternalFuncBtn, navBar.firstChild); 
		navBar.parentNode.insertBefore(ExternalFuncBtn, navBar.previousSibling);

		var ExternalFuncPopup = document.createElement('menupopup');
		ExternalFuncPopup.setAttribute('onpopupshowing', 'event.stopPropagation();gExternalFuncButton.onpopupshowing();');
		...
		setTimeout(function () { //延时加载菜单，不对启动造成影响，也不影响第一次打开菜单时的响应速度
			gExternalFuncButton.loadSubMenu();
		}, 3000);
		document.insertBefore(document.createProcessingInstruction('xml-stylesheet', 'type="text/css" href="data:text/css;utf-8,' + encodeURIComponent(
					'\
					#ExternalFuncButton-btn {\
					...
					#ExternalFuncButton-btn > .toolbarbutton-icon {\
						max-width: 18px !important;\
					    padding: 0 !important;\
					    margin: 0 !important;\
					}\
					#ExternalFuncButton-btn dropmarker{display: none !important;}\
	...
	loadSubMenu : function () {
		...
		for (var i = 0; i < this.toolbar.subdirs.length; i++) {
			if (this.toolbar.subdirs[i].name == 'separator') {
				...
				gExternalFuncButton.subdirPopupHash[this.toolbar.subdirs[i].name] = subDirItemPopup;
				gExternalFuncButton.subdirMenuHash[this.toolbar.subdirs[i].name] = subDirItem;
			}
		}
		...
		for (var i = 0; i < this.toolbar.configs.length; i++) {
			...
			if (this.toolbar.configs[i].subdir && gExternalFuncButton.subdirPopupHash[this.toolbar.configs[i].subdir])
				gExternalFuncButton.subdirPopupHash[this.toolbar.configs[i].subdir].appendChild(configItems);
			else
				ExternalFuncPopup.appendChild(configItems);
		}

		if (this.autohideEmptySubDirs) {
			for (let[name, popup]in Iterator(gExternalFuncButton.subdirPopupHash)) {
				if (popup.hasChildNodes()) {
					continue;
				} else {
					gExternalFuncButton.subdirMenuHash[name].setAttribute("hidden", "true");
				}
			}
		}
